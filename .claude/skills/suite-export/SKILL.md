...
One header row, then one row per case, in the twelve-column order the pipeline has always used, plus the three execution columns a tester needs to run the case, plus the five provenance columns that record where the case came from:

```
Test Case ID,Module,Title,Objective,Preconditions,Test Data,Steps,Expected Result,Priority,Tags,Layer,Entry Point,Oracle,Source,Reason for Extraction,Claude Model,Effort Level,User Prompt,Notes,Review,Automation
```

**Test Case ID** `TC-<MODULE>-<NNN>`, stable across re-runs; retire, never reuse. **Module** mandatory — what the tester filters on, a column here because there is only one file; a blank is a warning. **Title** short, specific, scannable. **Objective** one sentence. **Preconditions** only what must be true immediately before step 1. **Test Data** the concrete values the steps reference, one `field = value` per line, never "a valid X". **Steps** the full procedure, one action per numbered line, actions only. **Expected Result** the observable outcome constituting a pass: status, code, value, postcondition. **Priority** `P1`-`P4`. **Tags** exactly one granularity tier (`Atomic`/`Scenario`/`E2E`) plus the test types. **Layer** `API`, `UI`, `E2E` or `Contract`, nothing else. **Entry Point** exactly how the case is addressed: method+path, route, command, topic. **Oracle** `HTTPStatus`/`JSONBody`/`SchemaMatch`/`DBState`/`EventEmitted`/`LogEntry`/`UIAssertion`/`Snapshot`, combined with `+`. **Source** where this case came from: the `path:line` evidence it derives from, exactly as it appears in the report traceability row, multiple locations separated by `;` — and `Assumption: <gap ID>` when no code location backs it. **Reason for Extraction** why this case exists in one clause — the rule, boundary, failure mode, contract or journey that made it necessary (`Boundary: quantity max 99, upper-inclusive`, `EXT failure mode: payment gateway timeout`, `Authorization boundary: role customer vs admin`), never a restatement of the Objective. **Claude Model** the exact model ID that generated the case (`claude-opus-5`), not a family name. **Effort Level** the reasoning/effort setting the run used, or `Unknown` when the run cannot determine it — never a guess dressed as a fact. **User Prompt** the verbatim user request that initiated this generation run, newlines preserved inside the quoted cell; when the run was scoped per module, the prompt for that scope. **Notes** genuinely useful context only, naming the gap for any assumption-derived case. **Review** and **Automation** literally `Pending`, never anything else.

The last three are **run provenance** and therefore identical on every row of one run: they exist per row so a merged or filtered CSV stays self-describing and an audit can tell which run produced which case. Record the same three values once in the report Summary; a mismatch between the two is a warning.
...
2. Supply the promo code WINTER5.","The request is rejected with HTTP 422 and error code PROMO_EXPIRED; the payable total is unchanged at 100.00 and no order row is created.",P2,"Atomic, Negative-Temporal, BusinessRule",API,POST /api/checkout,HTTPStatus + JSONBody,"src/promotions/promo.service.ts:64; src/checkout/checkout.controller.ts:31","Business rule: a promo code is invalid once expiresAt is in the past",claude-opus-5,high,"Generate API test cases for the Checkout module from the codebase.","Expiry is compared against server time, not client time.",Pending,Pending
```
...
1. **Summary** — scope, repo, commit, generation date, **the run provenance (Claude model, effort level and the verbatim user prompt)**, total cases, counts by module / priority / tier / layer / test type
...
13. Every case has a non-empty **Source** — a case whose origin is blank cannot be re-derived or re-checked when the code moves; `Assumption: <gap ID>` is a valid Source, a blank is not
14. Every **Source** is a `path:line` that also appears in that case traceability row, or an `Assumption:` reference naming a gap in section 13 — a Source pointing somewhere the report does not corroborate is two claims that can disagree
15. Every case has a non-empty **Reason for Extraction** that names a rule, boundary, mode, contract or journey — "to test the endpoint" explains nothing and hides a case nobody chose deliberately
16. **Claude Model**, **Effort Level** and **User Prompt** are non-empty and identical on every row, and match the report Summary — provenance that varies row to row within one run means rows from different runs were merged without saying so
17. **Claude Model** is an exact model ID and **User Prompt** is the request verbatim — a paraphrased prompt or a family name makes the run unreproducible, which is the only thing these columns are for; where a value is genuinely unavailable write `Unknown` and warn, never invent one
...
**Never:** write the old intermediate Markdown "as well, just in case" (a dozen files that must agree is the problem this replaced) · drop a section because its source document no longer exists · flatten Steps onto one line to avoid quoting · omit the BOM · silently fix a case to avoid a warning · write `Review: Approved` (only a human moves that column) · paraphrase the `User Prompt` or fill provenance from memory rather than from the run · emit the CSV without the report (alone it is a list of assertions with no evidence, no denominator and no gaps) · leave a scratch file behind.
